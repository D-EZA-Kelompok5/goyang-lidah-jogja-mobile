...
List<WishlistElement> applyFilters(List<WishlistElement> wishlists) {
  ...
  // Apply custom min and max price filters if set
  if (minPrice != 0.0 || maxPrice != 1000000.0) {
    ...
    final selectedRange = priceRanges.firstWhere((range) => range['label'] == priceRange);
                ...
                child: DropdownButton<String>(
                  ...
                  items: <String>['lowest', 'highest'].map<DropdownMenuItem<String>>((String value) {
                    return DropdownMenuItem<String>(
                      value: value,
                      child: Text(value == 'lowest' ? 'Harga Terendah' : 'Harga Tertinggi'),
                    );
                  }).toList(),
                ),
              ),
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => WishlistEditPage(wishlist: item),
                          ),
                        ).then((_) {
                          setState(() {
                            futureWishlists = wishlistService.fetchWishlists(request);
