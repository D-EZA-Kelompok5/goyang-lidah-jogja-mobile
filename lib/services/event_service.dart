...
class EventService {
  ...
  Future<List<Event>> fetchEvents() async {
    final response = await request.get(
      'https://127.0.0.1:8000/event_manager/events_json/',
    );
  ...
  Future<void> createEvent(Event event) async {
    final response = await request.postJson(
      'https://127.0.0.1:8000/event_manager/create-event-flutter/',
  ...
  Future<void> updateEvent(Event event) async {
    final response = await request.postJson(
      'https://127.0.0.1:8000/event_manager/update-event-flutter/${event.id}/',
      jsonEncode(event.toJson()),
    );
  ...
  Future<void> deleteEvent(int eventId) async {
    final url = 'https://127.0.0.1:8000/event_manager/delete-event-flutter/$eventId/';
