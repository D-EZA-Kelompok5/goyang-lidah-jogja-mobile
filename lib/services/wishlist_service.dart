...
class WishlistService {
  ...
  Future<int> createWishlist(WishlistElement wishlist) async {
    ...
    final response = await request.postJson(
      'http://10.0.2.2:8000/wishlist/create-wishlist-flutter/',
      jsonEncode(data),
    );
  ...
  Future<void> updateWishlist(WishlistElement wishlist) async {
    ...
    final url =
        'http://10.0.2.2:8000/wishlist/update-wishlist-flutter/${wishlist.id}/';
    final response = await request.postJson(
      url,
      jsonEncode(data),
    );
  ...
  Future<void> deleteWishlist(int wishlistId, CookieRequest request) async {
    final url =
        'http://10.0.2.2:8000/wishlist/delete-wishlist-flutter/$wishlistId/';
